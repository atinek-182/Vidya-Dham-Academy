...
export const rayOpticsFrag = `
precision highp float;

uniform vec2 u_resolution;
uniform float u_time;
uniform float u_ray_angle;
uniform float u_n1;
uniform float u_n2;
...
void main() {
  ...
  vec3 color = vec3(0.02, 0.027, 0.047);

  if (st.y < 0.5) {
    color = mix(color, vec3(0.043, 0.059, 0.098), 0.6);
  }
